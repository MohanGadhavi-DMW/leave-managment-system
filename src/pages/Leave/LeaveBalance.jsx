        ...
        <div className="w-full flex-1 px-8 py-10 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 select-none">
                {leaveBalances.map((item) => (
                    <div
                        key={item.id}
                        className={`${item.type === "PRIVILEGED" ? "bg-[#FFF5ED]" : "bg-white"} ${item.type === "STUDY" ? "bg-[#F3F4F5]" : "bg-white"} rounded-xl p-6 flex flex-col relative`}
                    >
                        {/* Header */}
                        ...
                        <div className="mb-6">
                            <div className="flex items-baseline gap-1">
                                <span className={`text-4xl font-bold  ${item.type === "PRIVILEGED" ? "text-[#F6A500]" : ""}  ${item.type === "STUDY" ? "text-[#7E8795]" : ""}`}>
                                    {item.remaining}
                                </span>
                                <span className={`text-sm font-medium ${item.type === "PRIVILEGED" ? "text-[#F6A500]" : ""}  ${item.type === "STUDY" ? "text-[#7E8795]" : ""}`}>
                                    / Of {item.total} Days
                                </span>
                            </div>
                            <p className={`text-base text-black font-medium mt-1 `}>Remaining</p>
                        </div>

                        {/* Details */}
                        <div div className="space-y-3 pt-4 flex-1" >
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-gray-700 font-semibold tracking-wide">Accrual</span>
                                <span className="text-gray-900 font-semibold">{item.accrualType}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-gray-700 font-semibold tracking-wide">Carry Forward</span>
                                <span className="text-gray-900 font-semibold">{item.carryForward}</span>
                            </div>
                            ...
                            {/* Alerts */}
                            {
                                item.alert && (
                                    <div
                                        className={`mt-6 p-4 rounded-xl flex items-center gap-3 ${item.alert.type === "warning"
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-semibold">{item.alert.text}</span>
                                    </div>
                                )
                            }
                        </div>
            ...
            < div className="bg-white rounded-2xl border border-gray-100 py-6 px-6 mb-6 flex gap-6 items-start justify-center shadow-sm max-w-4xl" >
                ...
                <div>
                    <h4 className="text-black font-bold mb-1">Pro-tip for balance management</h4>
                    <p className="text-[#7E8795] text-base leading-relaxed">
                        Unused Annual Leave Balance (up to 5 days) will automatically carry forward to the 2025 period on
                        January 1st. Sick and Personal leave do not carry over
                    </p>
                </div>
            </div >
            ...
            < div className="bg-[#ACCBFD]/30 rounded-2xl border border-blue-100/50 p-6 flex gap-6 items-center shadow-sm max-w-6xl" >
                ...
                <div>
                    <h4 className="text-[#0A4A84] font-bold mb-1 uppercase tracking-wider text-sm">Upcoming Leave</h4>
